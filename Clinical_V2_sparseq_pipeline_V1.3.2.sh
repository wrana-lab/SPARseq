...
for f in $run_folder/R1_files/*.fastq; do bowtie $resource_folder/bowtie_index/clinical_V2_sparsq_V1p2_amplicons $f $f.sam --best -v 3 -k 1 -m 1 -S; done && for x in $run_folder/R1_files/*.sam; do python3 -m HTSeq.scripts.count -f sam -t CDS $x $resource_folder/bowtie_index/clinical_V2_sparsq_V1p2_amplicons.gtf > $x.count.txt; done;
mv $run_folder/R1_files/*.count.txt $run_folder/R1_files/samcounts

echo "	*Running analysis...*"
mkdir -p $run_folder/results
python3 $resource_folder/Clinical_V2_sparseq_analysis_V1.3.2.py $run_folder $resource_folder
